...
export class ForStatementRepresentation extends Representation {
    ...
    constructor(action: ActionState) {
        ...
        this.forLabelElement = createElement('div', [
            'action-proxy-code-label',
            'action-proxy-for-label',
        ])
        this.forLabelElement.innerText = 'for'
        monaco.editor
            .colorize(this.forLabelElement.innerHTML, 'javascript', {})
            .then((html) => {
    ...
    updateProxyVisual(proxy: ActionProxyState) {
        ...
        if (this.forLabelElement.parentElement != proxy.element) {
            ...
            // iterationElements[0].children[0].append(
            ...
            iterationElements[iterations].children[0].append(
