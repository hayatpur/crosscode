...
export class PartialCreateTrailRenderer extends TrailRenderer {
    /* ----------------------- Create ----------------------- */
    ...
    update(amount: number, environment: EnvironmentRenderer) {
        ...
        if (prev != null) {
            ...
            offset *= 5
            // const offset = 5 * (prev.element.parentElement.childElementCount - 2)

            prev.element.style.transform = `translate(${-pt - offset}px, ${pt + offset}px) scale(${
                1 - (pt + offset) / 35
            })`
            prev.element.style.opacity = `${Math.max(0.2, 1 - amount)}`
            // prev.element.style.opacity = `${Math.max(0, 1 - 0.8 * amount - (0.1 * offset) / 5)}`
