...
export class View {
  ...
  preChildrenContainer: HTMLDivElement;
  ...
  constructor(animation: AnimationGraph | AnimationNode, parent?: View) {
    ...
    this.preChildrenContainer = document.createElement("div");
    this.preChildrenContainer.classList.add("view-pre-children-container");
    ...
    if (parent != null) {
      ...
      labelEl.innerHTML = `[${animation.id}] ${camelCaseToSentence(
        animation.getName()
      )}`;
    ...
    this.element.append(this.preChildrenContainer);
    ...
    if (animation.showing && this.animation instanceof AnimationGraph) {
      ...
      Editor.instance.createLens(
        `[${this.animation.id}] ${camelCaseToSentence(
          this.animation.getName()
        )}`,
        startRow
      );
      this.codeSection = createCodeSection(this.animation);
    }
  ...
  tick() {
    if (this.parent == null) {
      ...
      this.animation instanceof AnimationGraph
    ) {
      // Then make sure it's next to the code block it represents
      const { bbox, startRow, endRow } = getLocation(this.animation);
      this.element.style.top = `${bbox.y - 10}px`;
    }
  ...
  update() {
    ...
    if (this.animation.showing) {
      ...
      this.beforeRenderer?.setState(this.animation.precondition);
    } else {
