...
export class Environment {
  ...
  copy() {
    ...
    copy.memory = this.memory.map((data) =>
      data != null ? data.copy() : null
    );
  ...
  isValid(animation: AnimationGraph | AnimationNode): boolean {
    ...
    // );
    return (
      [...this.validIds].filter((x) => animation.parentIds.has(x)).length > 0
    );
  }
  ...
  updateLayout(
    ...
    for (let i = 0; i < search.length; i++) {
      ...
      if (search[i].type == DataType.Array && search[i].frame >= 0) {
        ...
        position.x +=
          search[i].transform.width + (options?.isArrayElement ? 0 : 30);
      }
    }
  ...
  resolve(
    accessor: Accessor,
    _options: { noResolvingId?: boolean } = {}
  ): Data | Environment {
