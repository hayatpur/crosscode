...
export type FindMember = ExecutionNode & {
...
function apply(animation: FindMember, environment: EnvironmentState) {
    ...
    let original = resolvePath(
        ...
        [
            ...objectLocation,
            { type: AccessorType.Index, value: property.value as string },
        ],
    ...
    const objectReferenceLocation = getMemoryLocation(
        environment,
        objectReference
    ).foundLocation
    removeAt(
