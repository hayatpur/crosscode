...
export function computeEdges(
  ...
  for (let i = index - 1; i >= 0; i--) {
    ...
    for (const data of output) {
      if (output_added.some((existing) => existing.id == data.id) || i == index)
        continue;
...
function intersection(A: AnimationData[], B: AnimationData[]) {
  ...
  const satisfier = (a: AnimationData, b: AnimationData) => {
    ...
    return (
      b.id == a.id || JSON.stringify(b_location) == JSON.stringify(a_location)
    );
  };
...
export function logAnimation(
  ...
  // Flow edges
  for (const edge of animation.edges.filter(
    (edge) => edge instanceof FlowEdge
  )) {
  ...
  // Output-edges
  for (const edge of animation.edges.filter(
    (edge) => edge instanceof OutputEdge
  )) {
    // if (visited_edges.has(`${edge.to}_${edge.from}`)) continue;
  ...
  output = `${"\t".repeat(options.first ? 0 : indent)}[<reference>${
    animation.id
  }_${animation.node?.constructor.name}\n${output}`;
...
export function dissolve(animation: AnimationGraph) {
  ...
  if (
    ...
    animation.vertices[animation.vertices.length - 1] instanceof
      PopScopeAnimation
  ) {
