import { DataType, PrimitiveDataState } from '../../DataView/Environment/data/DataState'
import { clone } from '../../utilities/objects'
import { flattenedEnvironmentMemory, getMemoryLocation } from '../transpiler/environment'
...
export function getDepthOfExecution(target: ExecutionGraph | ExecutionNode, root: ExecutionGraph): number {
    ...
    while (true) {
        ...
        for (const vertex of parent.vertices) {
            ...
            if (instanceOfExecutionGraph(vertex) && Step.queryExecutionGraph(vertex, (a) => a.id == target.id)) {
                next = vertex
                break
            }
        }
