...
function computeReadAndWrites(animation: CopyReferenceAnimation, original: DataInfo, copy: DataInfo) {
    animation._reads = [original]
    animation._writes = [copy]
}

export function copyReferenceAnimation(dataSpecifier: Accessor[], outputRegister: Accessor[]): CopyReferenceAnimation {
    return {
