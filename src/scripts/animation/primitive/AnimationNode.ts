...
export class AnimationNode {
  ...
  reads() {
    if (this._reads == null) {
      console.error(
        `[${this.constructor.name}] Attempting to get reads from an AnimationNode before baking`
      );
    }
  ...
  writes() {
    if (this._writes == null) {
      console.error(
        `[${this.constructor.name}] Attempting to get writes from an AnimationNode before baking`
      );
    }
