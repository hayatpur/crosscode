...
export class CreateLiteralAnimation extends AnimationNode {
  ...
  begin(environment: Environment, options = { baking: false }) {
    ...
    environment._temps[`CreateLiteralAnimation${this.id}`] =
    ...
    if (options.baking) {
      this.computeReadAndWrites(
        ...
        {
          location: environment.getMemoryLocation(location).foundLocation,
          id: location.id,
        }
      );
    }
    ...
    const outputRegister = environment.resolvePath(this.outputRegister) as Data;
    outputRegister.replaceWith(new Data({ type: DataType.ID, value: data.id }));
  }

  seek(environment: Environment, time: number) {
    let t = super.ease(time / this.duration);
    const data = environment.resolvePath(
      environment._temps[`CreateLiteralAnimation${this.id}`]
    ) as Data;
  ...
  computeReadAndWrites(data: AnimationData, location: AnimationData) {
    console.log(location);
    this._reads = [location];
    this._writes = [data];
  }
}
